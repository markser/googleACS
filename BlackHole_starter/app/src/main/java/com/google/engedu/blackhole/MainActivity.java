...
public class MainActivity extends AppCompatActivity {
    ...
    // BlackHole Time
    // makes the unselected node change it's background into black
    private void blackHoleTime() {
        Log.d("BlackHole", "I'm being called on");
        Coordinates temp = new Coordinates();
        ArrayList<Integer> foo = new ArrayList<>();
        if (board.gameOver()) {
            for (int x = 0; x < board.tiles.length; x++) {
                if (board.tiles[x] == null) {
                    temp = board.indexToCoords(x);
                    int id = getResources().getIdentifier("button" + x, "id", getPackageName());
                    Button b = (Button) findViewById(id);
                    b.setText("?");
                    b.getBackground().setColorFilter(Color.BLACK, PorterDuff.Mode.MULTIPLY);
                }
            }
        }
        surroundingBlackHole(temp,foo);
    }

    //slowly removes the neighbouring nodes
    Handler handler1 = new Handler();
        private void surroundingBlackHole (Coordinates temp, ArrayList < Integer > foo){
            foo = board.getNeighboursCoordinates(temp);
            for (final int bar : foo) {
                handler1.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        int id = getResources().getIdentifier("button" + bar, "id", getPackageName());
                        Button b = (Button) findViewById(id);
                        b.setText("?");
                        b.getBackground().setColorFilter(null);
                    }
