...
public class ContinentMap extends View {
    ...
    private class Cell {
        public int height = 0;
        private boolean flowsNW = false;
        private boolean flowsSE = false;
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int tileSize = canvas.getWidth() / boardSize;
        for (int y = 0; y < boardSize; y++) {
            for (int x = 0; x < boardSize; x++) {
                ...
                paint.setARGB(
                        (int) Math.round(cell.height * 2 * (100D / (maxHeight * 2))), // a
                        cell.flowsNW ? 256 : 128, // r
                        cell.flowsSE ? 256 : 128, // g
                        128);
    ...
    private Cell buildDownContinentalDivideRecursively(int x, int y, int previousHeight) {
        ...
        if (x >= 0 && x < boardSize && y >= 0 && y < boardSize) {
            tempCell = getMap(x, y);
            if (!tempCell.processing && tempCell.height <= previousHeight) {
                tempCell.processing = true;
                tempCell.flowsSE = x == (boardSize - 1) || y == (boardSize - 1);
                tempCell.flowsNW = x == 0 || y == 0;

                int[][] coords = {{1, 0}, {0, 1}, {-1, 0}, {0, -1}};
                Cell[] cells = new Cell[4];
                for (int index = 0; index < coords.length; index++) {
                    int[] coord = coords[index];
                    cells[index] = buildDownContinentalDivideRecursively(x + coord[0], y + coord[1], tempCell.height);
                }

                for (int index = 0; index < cells.length; index++) {
                    tempCell.flowsSE |= cells[index].flowsSE;
                    tempCell.flowsNW |= cells[index].flowsNW;
                }

                tempCell.basin = !tempCell.flowsSE && !tempCell.flowsNW;

                for (int index = 0; index < cells.length; index++) {
                    if (cells[index].height == tempCell.height) {
                        cells[index].flowsSE |= tempCell.flowsSE;
                        cells[index].flowsNW |= tempCell.flowsNW;
                    }
                }
            }
        } else {
            tempCell.height = previousHeight;
        }
